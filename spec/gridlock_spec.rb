...
◯ ◯ ▢ ▢
    ...
    context '.each_symbol_of' do
      specify { expect { |b| game.each_symbol_of(%w[a b], &b) }.to yield_successive_args(['a', 0, 0], ['b', 0, 1]) }
      specify { expect { |b| game.each_symbol_of([['a', nil], ['b', nil]], &b) }.to yield_successive_args(['a', 0, 0], ['b', 1, 0]) }
      specify { expect { |b| game.each_symbol_of([['a', nil], %w[b c]], &b) }.to yield_successive_args(['a', 0, 0], ['b', 1, 0], ['c', 1, 1]) }
